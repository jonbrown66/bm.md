...
.xhs-article .xhs-key-point {
  margin: 30px 0 !important;
  padding: 28px 32px !important;
  border: 1px solid #e3d1ad !important;
  border-radius: 18px !important;
...
function XhsPage({
  ...
  return (
    <div
      ...
      className="relative overflow-hidden bg-white text-black shadow-sm"
      style={{
...
function fitsPage(probe: HTMLElement, html: string) {
  return getArticleHeight(probe, html) <= XHS_PAGE_HEIGHT + XHS_PAGE_HEIGHT_TOLERANCE
}
...
function enhanceXhsBlocks(blockRoot: HTMLElement) {
  Array.from(blockRoot.querySelectorAll('p')).forEach((paragraph) => {
    ...
    if (
      strongText.length >= 6
      && text.length <= 90
      && strongText.length / Math.max(text.length, 1) >= 0.5
    ) {
      paragraph.classList.add('xhs-key-point')
    }
  })
}
...
function buildSemanticPages(
  ...
  const pushElement = (element: HTMLElement) => {
    ...
    if (
      ...
      && XHS_PAGE_HEIGHT - getArticleHeight(probe, currentHtml.join('')) < XHS_MIN_TRAILING_SPACE
    ) {
      const trailingHeading = takeTrailingHeading()
      if (trailingHeading) {
        currentHtml = [trailingHeading]
          ...
          <div className="mx-auto flex w-full max-w-[580px] flex-col gap-8">
            {renderedPages.map((page, index) => (
              <div key={`preview-${page.id}`} className="flex flex-col gap-2">
                ...
                <div
                  className={`
                    origin-top overflow-hidden rounded-md border bg-white
                  `}
                  style={{
